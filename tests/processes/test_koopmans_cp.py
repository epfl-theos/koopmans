...
def test_convert_files_from_spin_1_to_2_process(tmpdir, check_patch, datadir):
    with utils.chdir(tmpdir):
        spin_1_files = [AbsoluteFilePointer(f) for f in (datadir / 'kcp' / 'spin_1_files').glob('*')]
        spin_2_up_files = (datadir / 'kcp' / 'spin_2_files').glob('*1.???')
        spin_2_down_files = (datadir / 'kcp' / 'spin_2_files').glob('*2.???')
...
def test_convert_files_from_spin_2_to_1_process(tmpdir, check_patch, datadir):
    with utils.chdir(tmpdir):
        spin_1_files = (datadir / 'kcp' / 'spin_1_files').glob('*')
        spin_2_files = [AbsoluteFilePointer(f) for f in (datadir / 'kcp' / 'spin_2_files').glob('*1.???')]
