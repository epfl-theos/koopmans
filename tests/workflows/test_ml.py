"""Test the Trajectory workflow for ML training."""
...
def test_ml_train_water(tmp_path, workflow_patch, water_snapshots, descriptor, estimator, occ_and_emp_together):
    """Test training various ML models on molecular water."""
    with chdir(tmp_path):
        projs = Projections.fromlist(
            [[{'site': 'O', 'ang_mtm': 'sp3'}], [{'site': 'H', 'ang_mtm': 's'}]],
            spins=[None, None],
            atoms=water_snapshots['atoms']
        )
        water_snapshots.pop('nbnd')
        water_snapshots['ecutwfc'] = 50.0
        wf = TrajectoryWorkflow(**water_snapshots, descriptor=descriptor, estimator=estimator,
                                occ_and_emp_together=occ_and_emp_together, train=True,
                                init_orbitals='mlwfs', init_empty_orbitals='mlwfs', projections=projs,
                                dis_froz_max=0.0, calculator_parameters={'pw': {'nbnd': 12}},
