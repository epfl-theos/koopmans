...
class KoopmansCPCalculator(CalculatorCanEnforceSpinSym, CalculatorExt, Espresso_kcp, CalculatorABC):
    ...
    def __init__(self, atoms: Atoms, alphas: Optional[List[List[float]]] = None,
        ...
        if has_nlcc and (self.parameters.nr1b is None or self.parameters.nr2b is None or self.parameters.nr3b is None):
            ...
            nr1 = np.sqrt(self.parameters.ecutrho) / (2.0 * np.pi / alat) * \
                np.sqrt(at[0, 0]**2 + at[0, 1]**2 + at[0, 2]**2) + 1
            nr1 = int(nr1) * 2
            nr2 = np.sqrt(self.parameters.ecutrho) / (2.0 * np.pi / alat) * \
                np.sqrt(at[1, 0]**2 + at[1, 1]**2 + at[1, 2]**2) + 1
            nr2 = int(nr2) * 2
            nr3 = np.sqrt(self.parameters.ecutrho) / (2.0 * np.pi / alat) * \
                np.sqrt(at[2, 0]**2 + at[2, 1]**2 + at[2, 2]**2) + 1
            nr3 = int(nr3) * 2
            ...
            nr3 = good_fft(nr3)
            #
            # At this stage nr should match with the one generated by CP or PW for the charge density. This can be a very
            # safe choice for nrb, but most probably too much.
            # If we have access to the PseudoPotential cutoff radius we can define a more reasonable one nr1b= nr1 * (2*rc/L1) 
            # where rc is the cutoff radius used to generate the PP and L1 is the dimension of the simulation Box
            # NOTA BENE: I assume here 3 Bohr is a safe choice: all the rc in the DOJO pseudos are <= 2.6: 
            ## 
            ## rcmax=0.0
            ## for file in `ls /data/colonna_n/CODES/GIT/koopmans/pseudos/pseudo_dojo_standard_v0.4.1/lda/*.upf`; do
            ## lmax=`grep -A 1 "\# lmax" $file | tail -1 | awk '{print $1+1}'`
            ## rc=`grep -A $lmax "l,   rc,      ep,       ncon, nbas, qcut" $file | grep -v "#" | awk '{print $2}' | sort -n | tail -1`
            ## echo $rc `echo $lmax | awk '{print $1-1}'` $file
            ## awk -v a=$rc -v b=$rcmax 'BEGIN {return_code=(a >= b) ? 0 : 1; exit} END {exit return_code}'
            ##  if [ $? -eq 0 ]; then
            ##  rcmax=$rc
            ##  echo NEW rc FOUND: $rcmax
            ## fi
            ## done
            ## echo "rmax =  " $rcmax
            #
            rc_safe=3.0
            L = np.sqrt(at[0, 0]**2 + at[1, 0]**2 + at[2, 0]**2)*alat
            nr1b = nr1 * ( 2*rc_safe/L)
            L = np.sqrt(at[0, 1]**2 + at[1, 1]**2 + at[2, 1]**2)*alat
            nr2b = nr2 * ( 2*rc_safe/L)
            L = np.sqrt(at[0, 2]**2 + at[1, 2]**2 + at[2, 2]**2)*alat
            nr3b = nr3 * ( 2*rc_safe/L)
            #
            ...
            print ("   Small box mesh      dimension ( nr1b, nr2b, nr3b )   = ", self.parameters.nr1b, self.parameters.nr2b, self.parameters.nr3b, "\n")
