...
!
!---------------------------------------------------------------
subroutine calc_delsdunrel(zed,grid,vxt,vh,vxc,nwf,nspin,delsdunrel,nunrel)   
  !---------------------------------------------------------------
  !
  use kinds, only : DP
...
!
     f5(i) =vxc(i,isw(nunrel))*rhoele+f1(i)+f2(i)+f4(i)
    enddo
!  The kinetic energy is the sum of the eigenvalues plus the f5 integral
!
  delsdunrel = int_0_inf_dr(f5,grid,grid%mesh,1)
  ...
  return
end subroutine calc_delsdunrel
