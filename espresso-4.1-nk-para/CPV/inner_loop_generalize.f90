...
!      integer         :: group1, group2 ! NsC 
      ...
      do nbnd1 = 1, nupdwn(isp)
         !
!         group1 = group(nbnd1,isp) ! NsC
!         WRITE(*,*) "NsC in inner_loop_general group1", group1 ! NsC
         !
         do nbnd2 = 1, nbnd1-1
            !
!            group2 = group(nbnd2,isp) ! NsC
!            WRITE(*,*) "NsC in inner_loop_general group1", group2 !NsC
            !
            if (lgam) then
               !  
	       ...
	       vsicah( nbnd1, nbnd2) = DBLE(hmat(nbnd1,nbnd2) -CONJG(hmat(nbnd2,nbnd1)))
               !
!               IF (group1 /= group2) vsicah( nbnd2, nbnd1)=0.0d0  ! NsC: mix only states in the same group 
!               IF (group1 /= group2) vsicah( nbnd1, nbnd2)=0.0d0  ! NsC: mix only states in the same group 
               !
            else
            !
