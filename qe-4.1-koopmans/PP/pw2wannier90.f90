...
program pw2wannier90
  ...
  logical :: split_orbital_twofiles, force_spin_symmetry

  ! these are in wannier module.....-> integer :: ispinw, ikstart, ikstop, iknum
  namelist / inputpp / outdir, prefix, spin_component, wan_mode, &
       seedname, write_unk, write_unkg, write_amn, write_mmn, write_spn, &
       wvfn_formatted, reduce_unk, nbnd_empty_to_save, empty_start_index, & 
       split_orbital_twofiles, force_spin_symmetry
  !
  ...
  call mp_bcast(force_spin_symmetry, ionode_id)
...
!     seedname='wannier'
  ...
  if(wan_mode.eq.'wannier2odd') then
     ...
     call read_nnkp
     call openfil_pp
     call wan2odd( seedname, nbnd_empty_to_save, iknum, ikstart, split_orbital_twofiles, spin_component, force_spin_symmetry)
     call stop_pp
     !
  endif
  !
  if(wan_mode.eq.'pw2cp_empty') then
     !
     call openfil_pp
     call pw2cp_empty ( seedname, iknum, ikstart, empty_start_index, nbnd_empty_to_save, spin_component, force_spin_symmetry )
     call stop_pp
     !
  endif
  !
...
SUBROUTINE write_parity
   ...
   USE mp_global,            ONLY : intra_pool_comm,mpime,nproc
   USE mp,                   ONLY : mp_sum
   USE io_global,            ONLY : stdout, ionode
   USE wvfct,                ONLY : nbnd
   ...
   !
   INTEGER                      :: npw,ibnd,igv,kgamma,ik,i,ig_idx(32),iun_parity
   ...
   !npw = ngk(kgamma) ! ORIGINAL uncommented
   ! NsC <<
   !
   ...
   DO igv=1,npw
!       g_abc(:,igk_k(igv,kgamma))=matmul(transpose(at),g(:,igk_k(igv,kgamma))) ! NsC back compatibility
