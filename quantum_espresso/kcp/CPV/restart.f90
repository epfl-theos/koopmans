...
!------------------------------------------------------------------------------!
      ...
      USE electrons_module, ONLY: nupdwn_emp, ei, ei_emp, nbsp_emp
      !
      IMPLICIT NONE
      !
...
!------------------------------------------------------------------------------!
      ...
      USE electrons_module, ONLY: nupdwn_emp, nbsp_emp, nbspx_emp, iupdwn_emp
      ...
      IF (nbsp_emp > 0) THEN
         !
         ALLOCATE (cemp(SIZE(c0, 1), nbspx_emp))
         cemp = 0.0d0
         t_emp = readempty(cemp, nbspx_emp, ndw)
         !
      END IF
      !
      IF (t_emp) THEN
      ...
      END IF
      !
      IF (nbsp_emp > 0) DEALLOCATE (cemp)
      !
      ! print evc0 of occ and empty in xml_io format
      !
      IF (print_evc0_occ_empty .and. (nbsp_emp > 0)) THEN
         !
         ...
         ALLOCATE (cemp(SIZE(c0, 1), nbspx_emp))
         cemp = 0.0d0
         t_emp = readempty_twin(cemp, nbspx_emp, ndw)
         !
         IF (t_emp) THEN
