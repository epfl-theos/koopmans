...
from koopmans.workflows import (ConvergenceVariable,
                                ConvergenceWorkflowFactory, DFTCPWorkflow,
...
subworkflow = DFTCPWorkflow(atoms=atoms, ecutwfc=30.0, base_functional='pbe',
...
# ... define a function that extracts nr1-3b from a workflow (we'll assume they're all the same)


def get_nrb(workflow):
    return [get_calculator_parameter(workflow, f'nr{i}b') for i in [1, 2, 3]]

# ... define a function that sets nr1-3b


def set_nrb(workflow, value):
    set_calculator_parameter(workflow, 'nr1b', value[0])
    set_calculator_parameter(workflow, 'nr2b', value[1])
    set_calculator_parameter(workflow, 'nr3b', value[2])


nrb_variable = ConvergenceVariable(name='nrb',
                                   increment=[2, 2, 2],
                                   get_value=get_nrb,
                                   set_value=set_nrb)
