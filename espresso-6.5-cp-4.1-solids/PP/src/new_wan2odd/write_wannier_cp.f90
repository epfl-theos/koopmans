...
!
!---------------------------------------------------------------------
SUBROUTINE write_wannier_cp( typ )
  !-------------------------------------------------------------------
  !
  ! ...  This routine reads the Wannier functions from the buffer
  ! ...  iunwann and writes them into a file readable by the
  ! ...  CP-Koopmans code
  ! 
  ...
  !
  USE kinds,               ONLY : DP
  USE buffers,             ONLY : get_buffer, open_buffer, close_buffer
  ...
  USE wannier2odd,         ONLY : iunwann, nwordwfcx, nrtot
  USE wannier,             ONLY : iknum, num_bands
  !
  !
  IMPLICIT NONE
  !
  CHARACTER(LEN=20), INTENT(IN) :: typ
  !
  LOGICAL :: opnd, exst
  INTEGER :: io_level = 1
  INTEGER :: cp_unit = 124
  ...
  !
  !
  INQUIRE( UNIT=iunwann, EXIST=exst, OPENED=opnd )
  !
  IF ( .not. exst ) &
    CALL errore( 'write_wannier_cp', 'Missing Wannier functions file', 1 )
  !
  IF ( .not. opnd ) &
  ...
  !
  IF ( typ .eq. 'occupied' ) THEN
    filename = 'evc_occupied.dat'
  ELSE IF ( typ .eq. 'empty' ) THEN
  ...
  !
  IF ( ionode ) THEN
    OPEN( UNIT=cp_unit, FILE=filename, STATUS='new', FORM='unformatted' )
    WRITE( cp_unit ) npw_g, num_bands*2
  ENDIF
  !
  ALLOCATE( evc_g(npw_g) )
  ALLOCATE( evc(npwxcp*npol,num_bands*2) )
  !
  DO ir = 1, nrtot
    !
    evc(:,:) = ( 0.D0, 0.D0 )
    ...
    DO ibnd = 1, num_bands*2
      ...
      !
      evc_g(:) = ( 0.D0, 0.D0 )
      CALL mergewf( evc(:,ibnd_), evc_g, npwxcp, ig_l2g_cp, &
  ...
  !
  IF ( .not. opnd ) CALL close_buffer( iunwann, 'keep' )
  !
  !
