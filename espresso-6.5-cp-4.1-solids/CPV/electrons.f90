   ...
   SUBROUTINE print_centers_spreads( spread_unit, tfile, tstdout, nfi, tps )
      ...
      DO j = 1, nspin
         ...
         IF( tstdout ) THEN
            ...
            !
            IF(do_orbdep) THEN
               WRITE(stdout,1444) ( i, wfc_centers(1:4, i, j), wfc_spreads( i, j, 1), wfc_spreads( i, j, 2), pink(iupdwn(j)-1+sort_spreads(i,j))*hartree_si/electronvolt_si, pzalpha(iupdwn(j)-1+sort_spreads(i,j)), i = 1, nupdwn(j) )
            ELSE
            ...
            IF( n_emp .GT. 0 ) THEN
               ...
               !
               WRITE( stdout,1446) ( i, wfc_centers_emp(1:4, i, j ), wfc_spreads_emp( i, j , 1), wfc_spreads_emp( i, j, 2), pink_emp(iupdwn_emp(j)-1+sort_spreads_emp(i,j))*hartree_si/electronvolt_si, pzalpha_emp(iupdwn_emp(j)-1+sort_spreads_emp(i,j)), i = 1, nupdwn_emp(j))
               !  
            ENDIF
            !  
         ENDIF
