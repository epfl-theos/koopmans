...
!=======================================================================
  ...
  !=========================================================!
    ...
    integer          :: ir, m, n,vec_idx, i, j, k, i_chosen, j_chosen, k_chosen
    real(kind=dp)    :: rdotk, dist2, min_dist2
    real(kind=dp),dimension(3)    :: dist_vec, min_distvec, irshift
    complex(kind=dp) :: phase_fac
    

    real(kind=dp), dimension(3,3) :: real_superlattice

    real_superlattice = real_lattice
    do vec_idx=1,3
       real_superlattice(vec_idx,:) = real_superlattice(vec_idx,:) * mp_grid(vec_idx)
    enddo
    ...
    do ir=1,nrpts
       do n=1, num_wann
          do m=1, num_wann
             ...
             irshift = matmul(real(irvec(:,ir),dp),real_lattice(:,:)) 
             ...
             do i=-2,2
                do j=-2,2
                   do k=-2,2
                      ! ang
                      ! Minus or plus for the 3rd term irshift?
                      dist_vec = wannier_centres(:,n) - wannier_centres(:,m) - &
                           irshift + &
                           i * real_superlattice(1,:) + &
                           j * real_superlattice(2,:) + &
                           k * real_superlattice(3,:)
                      dist2 = dot_product(dist_vec, dist_vec)
                      if (dist2 < min_dist2) then
                         min_dist2 = dist2
                         min_distvec = dist_vec
                         i_chosen = i
                         j_chosen = j
                         k_chosen = k
                      endif
             ...
             if (ndegen(ir) /= 1) then
                print*, "WARNING! ndegen!", ir, ndegen(ir)
             endif
             if (abs(i_chosen) + abs(j_chosen) + abs(k_chosen) /= 0) then
                print*, n,m,i_chosen, j_chosen, k_chosen,irvec(:,ir)
                print*, kpt, min_distvec
                print*, '-'
             endif

             !rdotk=twopi*dot_product(kpt(:),irvec(:,ir))
             !! Note I remove the twopi! Because recip_lattice is the inverse-transpose of real-lattice * twopi
             rdotk=dot_product(kpt(:),matmul(recip_lattice(:,:), min_distvec(:)))
             
             ! NOTE I don't think this is working together with ndegen!!
             ! To fix

             phase_fac=exp(cmplx_i*rdotk)/real(ndegen(ir),dp)
             if(alpha==0) then
                OO(m,n)=OO(m,n)+phase_fac*OO_R(m,n,ir)
             elseif(alpha==1.or.alpha==2.or.alpha==3) then
                OO(m,n)=OO(m,n)+&
                     cmplx_i*crvec(alpha,ir)*phase_fac*OO_R(m,n,ir)
             else
