#! /bin/bash
#
# Calculation of empty states (clorine mol, nspin=1)
# 
#================================================================
#
...
MANUAL=" Usage
 ...
 pbe0_empty      PBE0 empty states
 b3lyp           B3LYP calculation
 b3lyp_empty     B3LYP empty states
...
case $INPUT in 
   ...
   (pbe0)           PBE0=yes ;;
   (pbe0_empty)     PBE0_EMPTY=yes ;;
   (b3lyp)          B3LYP=yes ;;
   (b3lyp_empty)    B3LYP_EMPTY=yes ;;

   (odd)            NK0=yes  ; NK=yes; PZ=yes; HF=yes ;
                    NK0_EMPTY=yes ; NK_EMPTY=yes; PZ_EMPTY=yes; HF_EMPTY=yes ;
                    PBE0=yes ; PBE0_EMPTY=yes ; B3LYP=yes ; B3LYP_EMPTY=yes ;;
...
#
if [ "$LDA_EMPTY" = "yes" -a -e ./SCRATCH/clorine.emp ] ; then
    rm -f ./SCRATCH/clorine.emp 2> /dev/null
fi
...
run_cp  NAME=PBE0_EMPTY   SUFFIX=$SUFFIX  RUN=$PBE0_EMPTY
#
run_cp  NAME=B3LYP        SUFFIX=$SUFFIX  RUN=$B3LYP
run_cp  NAME=B3LYP_EMPTY  SUFFIX=$SUFFIX  RUN=$B3LYP_EMPTY
