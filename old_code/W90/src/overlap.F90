...
!DO i =1, num_wann
!write(*,*) i,j,cmplx(u_matrix_r(i,j), 0.0_dp, kind=dp) 
!u_matrix(i,:,1) = cmplx(u_matrix_r(i,:), 0.0_dp, kind=dp)
!ENDDO
!stop
    !u_matrix(:,:,1) = cmplx(u_matrix_r(:,:), 0.0_dp, kind=dp)
    u_matrix(:,:,1) = dcmplx(u_matrix_r(:,:), 0.0_dp)
!DO i =1, num_wann
!DO j =1, num_wann
!write(*,*) i,j,u_matrix(i,j,1), dcmplx(u_matrix_r(i,j), 0.0_dp), cmplx(u_matrix_r(i,j), 0.0_dp,kind=dp)
!ENDDO
!ENDDO
