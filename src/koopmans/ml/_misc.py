...
def compute_3d_integral_naive(f: np.ndarray, r: np.ndarray) -> np.ndarray:
    ...
    y = r[0, :, 0, 1]
    x = r[0, 0, :, 2]
    result_n = np.sum(f, axis=(0, 1, 2)) * (x[-1] - x[0]) * (y[-1] - y[0]) * (z[-1] - z[0]) \
        / ((len(x) - 1) * (len(y) - 1) * (len(z) - 1))
    return result_n


def reconstruct_orbital_densities(total_basis_array: np.ndarray, coefficients: List[float]) -> np.ndarray:
    """Reconstruct the density with the truncated expansion coefficients multiplied with the basis functions."""
    return np.einsum('ijkl,l->ijk', total_basis_array, coefficients)


def map_to_original_grid(array: np.ndarray, wfc_center_index: Tuple[int, int, int], nr_xml: Tuple[int, int, int],
                         nr_new: Tuple[int, int, int]):
    """Map the array defined on the new integration domain back to the unit cell."""
